...
function NavItem({ to, label, icon: Icon, mobile = false }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        mobile
          ? `flex flex-col items-center gap-0.5 px-2 py-1 rounded-lg text-xs font-medium transition-colors min-w-[52px] ${
              isActive ? 'text-success' : 'text-muted hover:text-ink'
            }`
          : `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${
              isActive
                ? 'bg-success text-white'
                : 'text-slate-400 hover:bg-nav-hover hover:text-white'
            }`
      }
    >
      ...
      <div className="px-4 py-5 border-b border-slate-800">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-slate-500 text-[9px] font-medium tracking-widest uppercase">Debt Copilot</div>
          </div>
        </div>
      </div>
